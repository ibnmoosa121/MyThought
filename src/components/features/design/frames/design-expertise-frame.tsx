...
const ExpertiseItem = ({ item }: { item: { title: string, icon: any, desc: string } }) => {
    ...
    return (
        ...
        >
            ...
            <div>
                <h4 className="text-lg md:text-xl font-bold text-white uppercase italic tracking-tight">{item.title}</h4>
                <p ref={descRef} className="text-zinc-500 text-base md:text-sm leading-tight opacity-0 h-0 overflow-hidden">{item.desc}</p>
            </div>
        </div>
    );
};
        ...
        <section ref={containerRef} className="relative w-full min-h-screen py-16 md:py-32 flex items-center justify-center bg-transparent overflow-hidden px-6 lg:px-20">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center w-full max-w-7xl relative z-10">
                <div className="flex flex-col">
                    ...
                    <p className="text-zinc-200 text-[clamp(1rem,2vw,1.5rem)] max-w-xl leading-relaxed font-medium tracking-tight">
